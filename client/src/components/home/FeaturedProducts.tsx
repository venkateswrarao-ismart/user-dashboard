      ...
      <div className="container mx-auto px-4">
        ...
        <div className="flex justify-between items-center mb-8">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            {/* NAVIGATION BUTTONS */}
            <button onClick={handlePrev} disabled={currentPage === 0}>
              <ChevronLeft className="w-8 h-8 sm:w-10 sm:h-10 bg-gray-100 rounded-full p-2 hover:bg-gray-200" />
            </button>

            <button onClick={handleNext} disabled={currentPage === totalPages - 1}>
              <ChevronRight className="w-8 h-8 sm:w-10 sm:h-10 bg-gray-100 rounded-full p-2 hover:bg-gray-200" />
            </button>
          </div>
        </div>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {currentProducts.map((product) => (
              ...
              >
                <Link to={`/products/${product.id}`}>
                  <div className="bg-white rounded-lg shadow-md p-3 md:p-4 flex flex-col h-full hover:shadow-lg transition">
                    ...
                    <div className="mt-auto text-sm sm:text-base">
                      {product.selling_price < product.price ? (
                        <>
                          ...
                          <span className="line-through text-gray-500 ml-2">₹{product.price}</span>
                          <Badge className="bg-red-500 ml-2 text-xs sm:text-sm">
                            {Math.round((product.price - product.selling_price) / product.price * 100)}% OFF
                          </Badge>
                        </>
                      ) : (
